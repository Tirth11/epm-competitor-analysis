...
function inferCategory(heading, name, description) {
...
function parseFeatureFromElement($element) {
...
function collectReferences($element, pageUrl) {
    const urls = new Set();
...
function pickPrimarySource(references, pageUrl) {
    const youtube = references.find((ref) => isYoutube(ref));
    if (youtube)
        return youtube;
    return references[0] ?? pageUrl;
}
function pushFeature(features, seen, feature) {
...
function extractFeaturesFromPage(html, pageUrl) {
...
function shouldCheckSource(source, force) {
    if (force)
        return true;
...
function syncProductFeatures(productId, productName, pageUrl, extracted) {
  ...
  `);
    const tx = db.transaction(() => {
        for (const feature of extracted) {
            ...
            if (!existingFeature) {
                const result = insertFeature.run(productId, feature.name, feature.category, feature.subCategory, feature.description, primarySource, now, now, 'new');
                ...
                insertUpdate.run(productId, featureId, 'added', `${productName}: feature added`, feature.name, primarySource);
                continue;
            }
            const changed = existingFeature.category !== feature.category ||
            ...
            if (changed) {
                ...
                updateFeature.run(feature.category, feature.subCategory, feature.description, primarySource, now, changeType, existingFeature.id);
                insertUpdate.run(productId, existingFeature.id, changeType === 'new' ? 'added' : 'changed', `${productName}: feature ${changeType === 'new' ? 'reintroduced' : 'updated'}`, feature.name, primarySource);
            }
        ...
        for (const existingFeature of existing) {
            ...
            if (!seen.has(key) && existingFeature.change_type !== 'removed') {
                ...
                insertUpdate.run(productId, existingFeature.id, 'deprecated', `${productName}: feature removed`, existingFeature.name, existingFeature.source_url ?? pageUrl);
            }
        }
    });
    tx();
}
export async function runCompetitorCheck(options = {}) {
  ...
  `);
    for (const source of sources) {
        ...
        try {
            ...
            const extractedFeatures = source.source_type === 'product_page' ? extractFeaturesFromPage(html, source.url) : [];
