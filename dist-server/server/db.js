...
function migrateFeaturesTable() {
    ...
    const needsMigration = columnNames.has('notes') ||
    ...
    BEGIN;
      ...
      INSERT INTO features_new (
        id, product_id, name, category, sub_category, status,
        description, source_url, first_detected, last_updated, change_type
      )
      SELECT
        id, product_id, name, category, sub_category,
        ${selectStatus}, ${selectDescription}, ${selectSourceUrl},
        ${selectFirstDetected}, COALESCE(last_updated, CURRENT_TIMESTAMP), ${selectChangeType}
      FROM features;
      DROP TABLE features;
