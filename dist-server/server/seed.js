...
const upsertProduct = (product) => {
    ...
    const row = db.prepare('SELECT id FROM products WHERE name = ?').get(product.name);
    if (!row) throw new Error(`Failed to upsert product ${product.name}`);
    return row.id;
};

const replaceSources = (productId, product) => {
...
const syncFeatures = (productId, productName, features) => {
    ...
    const updateFeature = db.prepare(`
      UPDATE features SET category = ?, sub_category = ?, status = ?, description = ?, source_url = ?, last_updated = ?, change_type = ? WHERE id = ?
    `);
    const markRemoved = db.prepare(`
      UPDATE features SET status = 'Deprecated', change_type = 'removed', last_updated = ? WHERE id = ?
    `);
    const insertUpdate = db.prepare(`
      INSERT INTO updates (product_id, feature_id, update_type, title, detail, source_url) VALUES (?, ?, ?, ?, ?, ?)
    `);
    const deleteFeatureSources = db.prepare('DELETE FROM feature_sources WHERE feature_id = ?');
    const insertFeatureSource = db.prepare(`
      INSERT OR IGNORE INTO feature_sources (feature_id, source_type, url) VALUES (?, ?, ?)
    `);
    ...
    for (const feature of features) {
        ...
        if (existingRow) {
            const changed = existingRow.category !== feature.category ||
            ...
            if (changed) {
                ...
                updateFeature.run(feature.category, subCategory, status, description, primary, now, changeType, existingRow.id);
                insertUpdate.run(productId, existingRow.id, changeType === 'new' ? 'added' : 'changed',
                    `${productName}: ${feature.name} ${changeType === 'new' ? 'reintroduced' : 'updated'}`, description, primary);
            }
            deleteFeatureSources.run(existingRow.id);
            for (const url of refs) { insertFeatureSource.run(existingRow.id, 'reference', url); }
            continue;
        }

        const result = insertFeature.run(productId, feature.name, feature.category, subCategory, status, description, primary, now, now, 'new');
        const featureId = Number(result.lastInsertRowid);
        for (const url of refs) { insertFeatureSource.run(featureId, 'reference', url); }
        insertUpdate.run(productId, featureId, 'added', `${productName}: ${feature.name} added`, description, primary);
    }

    for (const row of existing) {
        if (!seen.has(row.name.toLowerCase()) && row.change_type !== 'removed') {
            markRemoved.run(now, row.id);
            insertUpdate.run(productId, row.id, 'deprecated', `${productName}: ${row.name} removed`, row.description ?? null, row.source_url ?? null);
        }
    }
};

const syncUseCases = (productId, useCases) => {
    db.prepare('DELETE FROM use_cases WHERE product_id = ?').run(productId);
    const insert = db.prepare(`
      INSERT INTO use_cases (product_id, title, category, industry, problem, solution, outcome, source_url)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `);
    for (const uc of useCases) {
        insert.run(productId, uc.title, uc.category, uc.industry ?? null, uc.problem, uc.solution, uc.outcome, uc.sourceUrl ?? null);
    }
};

const removeStaleProducts = (activeNames) => {
...
export function applyArconPrimaryProfile() {
    // No-op: ARCON|EPM is seeded via the unified catalog.
}
