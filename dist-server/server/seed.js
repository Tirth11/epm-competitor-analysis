...
export function applyArconPrimaryProfile() {
    ...
    const hasSource = db.prepare('SELECT id FROM sources WHERE product_id = ? AND source_type = ?').get(arcon.id, 'product_page');
    if (!hasSource) {
  ...
  `);
    for (const feature of arconFeatures) {
      ...
      `).run(feature.category, feature.subCategory, 'https://arconnet.com', now, existing.id);
        ...
        const result = insertFeature.run(arcon.id, feature.name, feature.category, feature.subCategory, null, 'https://arconnet.com', now, now, 'new');
