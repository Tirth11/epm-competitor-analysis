...
type ProductSeed = { name: string; website: string; isPrimary?: boolean };
...
export function seedData() {
  ...
  const insertProduct = db.prepare(
    'INSERT INTO products (name, website, is_primary) VALUES (@name, @website, @isPrimary)'
  );
  ...
  for (const p of products) {
    ...
    insertUpdate.run(
      productId,
      'announcement',
      `${p.name} baseline profile added`,
      'Initial competitor profile seeded for tracking and benchmarking.',
      p.website
    );
  }
}
  ...
  for (const feature of arconFeatures) {
    ...
    if (existing) {
      ...
      `).run(
        feature.category,
        feature.subCategory,
        'Provided by ARCON feature input.',
        now,
        existing.id
      );
      continue;
